    ...
    <aside :class="[
      ...
      <div class="flex items-center gap-3 px-4 py-5 border-b border-gray-100">
        ...
        <div class="overflow-hidden" :class="store.sidebarOpen ? 'lg:block' : 'lg:hidden'">
          <p class="font-bold text-gray-900 text-sm leading-tight">Teacher Portal</p>
      ...
      <div class="border-t border-gray-100 p-3">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0">
            <span class="text-xs font-bold text-emerald-700">{{ auth.user?.avatar }}</span>
          </div>
...
import { IconDashboard, IconAttendance, IconAcademics, IconTimetable, IconCommunication, IconLogout, IconMenu, IconGrades } from '@/components/icons'
...
const navItems = [
  ...
  { to: '/teacher/grades', label: 'Gradebook', icon: IconGrades },
  { to: '/teacher/assignments', label: 'Assignments', icon: IconAcademics },
  { to: '/teacher/timetable', label: 'My Timetable', icon: IconTimetable },
  { to: '/teacher/communication', label: 'Messages', icon: IconCommunication },
]
